resource "azurerm_network_security_group" "nsg" {
  ...
  # btw, dynamic block => dynamic "rule_name"
  dynamic "security_rule" {
    ...
    content {
      ...
      source_address_prefix      = "AzureLoadBalancer"                  # only the load balancer
...
azurerm_subnet_network_security_group_association is what actually applies that NSG to a specific subnet, turning it from "rules sitting unused" into "rules actively filtering traffic */
